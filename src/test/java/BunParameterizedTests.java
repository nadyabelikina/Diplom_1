import org.junit.Assert;
import org.junit.runner.RunWith;
import org.junit.runners.Parameterized;
import org.junit.Test;
import praktikum.Bun;


@RunWith(Parameterized.class)
public class BunParameterizedTests {
    private final String name;
    private final float price;

    public BunParameterizedTests(String name, float price) {
        this.name = name;
        this.price = price;
    }

    @Parameterized.Parameters(name = "Тестовые данные: {0}, {1}")
    public static Object[][] getData() {
        return new Object[][]{
                {"Флюоресцентная булка R2-D3", 988},
                {"Краторная булка N-200i", 1255},
                //пустое поле
                {"", 988},
                //названия булки со значением null
                {null, 988},
                //отрицательная цена
                {"Флюоресцентная булка R2-D3", -988},
                //цена равна нулю
                {"Краторная булка N-200i", 0},
                //максимально возможная цена
                {"Флюоресцентная булка R2-D3", Float.MAX_VALUE},
                //минимально возможная цена
                {"Краторная булка N-200i", Float.MIN_VALUE},
                //цена, которая содержит нечисловое значение
                {"Флюоресцентная булка R2-D3", Float.NaN},
                //цена, которая содержит значение с плавающей точкой близкое к минимально возможному значению
                {"Краторная булка N-200i", 1.4E-45f},
                //цена, которая содержит значение с плавающей точкой близкое к максимально возможному значению
                {"Флюоресцентная булка R2-D3", 3.40282347E38f},
                //Проверка специальных символов в строке
                {"#$%^&*", 0}
        };
    }
    @Test
    public void getNameBunParameterizedTest() {
        Bun bun = new Bun(name, price);
        Assert.assertEquals(name, bun.getName());
    }

    @Test
    public void getPriceBunParameterizedTest(){
        Bun bun = new Bun(name, price);
        Assert.assertEquals(price, bun.getPrice(), 0);
    }
}

